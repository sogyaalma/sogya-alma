...
export default function Header() {
  ...
  return (
    <header className={`bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 sticky top-0 z-50 h-[72px] flex items-center transition-all duration-300 ${isScrolled ? 'shadow-lg' : 'shadow-sm'}`}>
      <div className="container mx-auto px-4 md:px-0 w-full flex justify-between items-center h-full max-w-7xl">
        ...
        <div className="flex items-center justify-end h-full">
          ...
          <div className="hidden lg:flex items-center gap-2 relative pl-4 border-l border-gray-100 dark:border-gray-700">
            {showSearch && (
              <div className="absolute top-14 left-0 bg-white dark:bg-gray-800 shadow-lg border border-gray-100 dark:border-gray-700 rounded-xl p-2 w-64 z-50 animate-in fade-in zoom-in duration-200">
                <form onSubmit={handleSearch}>
                  <input 
                    type="text" 
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="ابحث في الفرص، المشاريع..." 
                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg py-2 px-4 outline-none focus:border-primary text-sm dark:text-white" 
                    autoFocus 
                  />
                </form>
              </div>
            ...
            <button 
              ...
              className="p-2 rounded-full transition-colors text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-primary dark:text-gray-400"
            >
              <Sun size={20} className="hidden dark:block" />
              <Moon size={20} className="block dark:hidden" />
            </button>

            <button onClick={toggleLang} className="flex items-center gap-1 text-gray-500 dark:text-gray-400 hover:text-primary transition-colors text-sm font-bold bg-gray-50 dark:bg-gray-800 px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-700">
              <Globe size={18} />
          ...
          <div className="flex items-center gap-2 pr-4 relative">
            <Link href="/cart" className="relative text-primary hover:text-primary-dark transition-colors p-2">
              ...
              <span className="absolute top-0 right-0 bg-red-600 text-white text-[11px] font-bold w-4 h-4 flex items-center justify-center rounded-full pointer-events-none translate-x-1 -translate-y-1">
                0
              </span>
            </Link>
            ...
            <div className="hidden md:flex items-center h-full relative" id="login-btn">
              {user ? (
                ...
                <Link href="/auth/login" className="flex items-center gap-2 bg-primary/10 text-primary hover:bg-primary hover:text-white transition-colors font-bold text-[14px] px-4 py-2 rounded-full mr-2">
                  <User size={18} strokeWidth={2} />
                  تسجيل الدخول
                </Link>
              )}
        ...
        <div className="md:hidden fixed inset-0 top-[72px] z-40">
          ...
          <nav className="absolute top-0 right-0 w-[280px] h-full bg-white dark:bg-gray-900 shadow-2xl border-l border-gray-100 dark:border-gray-800 overflow-y-auto">
            <div className="flex flex-col p-6 gap-1">
              ...
              {/* Mobile Search */}
              <form onSubmit={(e) => { e.preventDefault(); if(searchQuery.trim()) { router.push(`/opportunities?search=${encodeURIComponent(searchQuery)}`); setIsMobileMenuOpen(false); } }} className="px-1">
                <input 
                  type="text" 
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="ابحث في المشاريع..." 
                  className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 px-4 outline-none focus:border-primary text-sm dark:text-white" 
                />
              </form>
